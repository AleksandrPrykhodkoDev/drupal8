 ...
 *
 * @ViewsField("delete_staff")
 */
class DeleteStaff extends FieldPluginBase {
  ...
  public function render(ResultRow $values) {
    $result = [];
    $nid = $values->nid;
    if ($values->users_field_data_node__db_staff_uid) {
      $uid = $values->users_field_data_node__db_staff_uid;

      $result['remove'] = [
        '#type' => 'link',
        '#title' => '',
        '#url' => Url::fromRoute('crm_department_balance.remove_staff_balance', [
          'nid' => $nid,
          'uid' => $uid,
        ]),
        '#attributes' => [
