...
/**
 * Class RemoveDepartmentForm.
 */
class RemoveDepartmentForm extends FormBase {
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    if ($trigerring_element['#name'] !== 'cancel') {
      ...
      $departments_balance = $department_balance->get('db_departments')->getValue();
      $departments = [];
      foreach ($departments_balance as $department) {
        $departments[$department['target_id']] = $department['target_id'];
      }
      unset($departments[$form_state->getValue('department_id')]);
      $department_balance->set('db_departments', $departments);
      $department_balance->save();
      $log_message = $this->t('Departments were deleted from Department Balance - @name', [
        '@name' => $department_balance->get('db_balance_name')->value,
      ]);
      $this->actionLog->log('remove_department_from_department_balance', $log_message, $this->currentUser->id());
      $this->messenger()->addStatus($this->t('Department was successfully deleted'));
    }
  }
