...
class DepartmentSettingsForm extends FormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state, $nid = NULL) {
    ...
    $form['departments'] = [
      ...
      '#options' => $this->departmentBalanceService->getDepartmentSettings($nid, FALSE),
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $departments_input = $form_state->getValue('departments');
    $department_balance = $this->entityTypeManager->getStorage('node')->load($balance_id);
    $departments_balance = $this->departmentBalanceService->getDepartmentsInBalance($balance_id, FALSE);
    $departments = $departments_input + array_flip($departments_balance);
    $department_balance->set('db_departments', $departments);
    $department_balance->save();
    $log_message = $this->t('Departments were added to Department Balance - @name', [
      '@name' => $department_balance->get('db_balance_name')->value,
    ]);
    $this->actionLog->log('add_department_to_department_balance', $log_message, $this->currentUser->id());
    $this->messenger()
      ->addStatus($this->t('Departments for balance was successfully added'));
  }
