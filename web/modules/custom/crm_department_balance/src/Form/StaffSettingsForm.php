...
/**
 * Class StaffSettingsForm.
 */
class StaffSettingsForm extends FormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state, $nid = NULL) {
    ...
    $form['staff'] = [
      ...
      '#size' => 5,
      '#title' => $this->t('Staff'),
      '#options' => $this->departmentBalanceService->getStaffSettings($nid),
      '#description' => $this->t('Select a staff to link it to the balance'),
      '#required' => TRUE,
    ];
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $staff_input = $form_state->getValue('staff');
    $department_balance = $this->entityTypeManager->getStorage('node')->load($balance_id);
    $staff_balance = $this->departmentBalanceService->getStaff($balance_id);
    $staff = $staff_input + array_flip($staff_balance);
    $department_balance->set('db_staff', $staff);
    $department_balance->save();
    $log_message = $this->t('Staff were added to Department Balance - @name', [
      '@name' => $department_balance->get('db_balance_name')->value,
    ]);
    $this->actionLog->log('add_staff_to_department_balance', $log_message, $this->currentUser->id());
    $this->messenger()
      ->addStatus($this->t('Staff for balance was successfully added'));
  }
