 ...
 */
class NunavutVideoFileUploadForm extends AddFormBase {
  ...
  protected function getMediaType(FormStateInterface $form_state) {
    ...
    if (!is_a($field_definition->getClass(), FileFieldItemList::class, TRUE)) {
      throw new \InvalidArgumentException(
        'Can only add media types which use a file field as a source field.'
      );
    }
  ...
  protected function buildInputElement(array $form, FormStateInterface $form_state) {
    ...
    $form['container']['upload']['#description'] = $this
      ->renderer
  ...
  public function validateUploadElement(array $element, FormStateInterface $form_state) {
    ...
    if (
      count($values['fids']) > $element['#cardinality']
      && $element['#cardinality'] !== FieldStorageDefinitionInterface::CARDINALITY_UNLIMITED
    ) {
      $form_state->setError(
        $element,
        $this->t('A maximum of @count files can be uploaded.', [
   ...
   * {@inheritdoc}
   */
  protected function buildEntityFormElement(
    MediaInterface $media,
    array $form,
    FormStateInterface $form_state,
    $delta
  ) {
    $element = parent::buildEntityFormElement(
      $media,
      $form,
      $form_state,
      $delta
    ...
    $source_field = $this->getSourceFieldName(
      $media->bundle->entity
    );
   ...
   */
  public static function hideExtraSourceFieldComponents(
    array $element,
    FormStateInterface $form_state,
    array $form
  ): array {
  ...
  protected function createMediaFromValue(
    ...
    if (!($file instanceof FileInterface)) {
      throw new \InvalidArgumentException(
        'Cannot create a media item without a file entity.'
      );
    }
    ...
    if (!$this->fileSystem->prepareDirectory($upload_location, FileSystemInterface::CREATE_DIRECTORY)) {
      throw new FileWriteException(
        "The destination directory '$upload_location' is not writable"
      );
    }

    if (!($file = file_move($file, $upload_location))) {
      throw new \RuntimeException(
        "Unable to move file to '$upload_location'"
      );
    }

    return parent::createMediaFromValue(
      $media_type,
      $media_storage,
      $source_field_name,
      $file
  ...
  public function removeButtonSubmit(array $form, FormStateInterface $form_state) {
    ...
    $file = $removed_media
      ->get($this->getSourceFieldName($removed_media->bundle->entity))
      ->entity;
  ...
  public function validateUrl(array &$form, FormStateInterface $form_state) {
    if ($url = $form_state->getValue('url')) {
      try {
