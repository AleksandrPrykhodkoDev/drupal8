...
class ColorButton extends CKEditorPluginBase implements CKEditorPluginConfigurableInterface {
  ...
  public function getLibraryPath() {
    return 'libraries/ckeditor/plugins/colorbutton';
  }
  ...
  public function validateInput(array $element, FormStateInterface $form_state) {
    ...
    if (preg_match('/([^A-F0-9,])/i', $input)) {
      $form_state->setError(
        $element,
        'Only valid hex values are allowed (A-F, 0-9). No other symbols or letters are allowed. Please check your settings and try again.'
      );
    }
  }
