...
class LocationSearchController extends ControllerBase {
  ...
  public function searchPage($province, $city) {
    ...
    $title = t('Smallsteps locaties in jouw buurt');
  ...
  private function searchPageContent($title, $button, $search, $type, $route_name = NULL, $page = 'search', $city = NULL) {
    ...
    $results = [];
    $count = 0;
    $range = (isset($_GET['range']) && is_numeric($_GET['range']) && $_GET['range'] > 0) ? $_GET['range'] : 0;
    if (isset($search, $type)) {
      ...
      $attached['drupalSettings']['ss_location']['search']['range'] = $range;
      ...
      $results = $this->nearest($result['Latitude'], $result['Longitude'], $services, $referer, $button, $range, 0, TRUE);
    }
  ...
  public function nearest($latitude = NULL, $longitude = NULL, $services = [], $referer = 'canonical', $button = 'Ontdek deze locatie', $range = 0, $page = 0, $return = FALSE) {
    ...
    $range = isset($_GET['range']) ? $_GET['range'] : $range;

    $results = [];
    if ($latitude && $longitude) {
      $results = ss_location_order_by_nearest($latitude, $longitude, $services, $range, $page);
    }
