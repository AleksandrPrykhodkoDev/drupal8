...
class LocationSearchForm extends FormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state, $referer = 'canonical', $page = 'search') {
    ...
    $form['search'] = [
      '#type' => 'textfield',
      '#attributes' => [
        'placeholder' => ($type == 'locatienaam') ? t('zoek op locatienaam') : t('postcode of plaats'),
      ],
    ...
    if (in_array($location_forms, $form)) {
      $form['search']['#attributes']['placeholder'] = ($type == 'locatienaam') ? t('zoek op locatienaam') : t('Zoek op locatienaam');
    }
