...
class LocationSearch {
  ...
  public function getLocationByName($name) {
    ...
    $query->leftJoin('smallsteps_test_dashboard.Cities', 'c', 'c.Id = l.City');
  ...
  public function getLocationByAddress($address) {
    ...
    $query = \Drupal::database()
      ->select('smallsteps_test_dashboard.Postcodes', 'p');
    $query->fields('p', ['Id']);
    ...
    if (strpos($address, ',') !== FALSE) {
      list($address, $province) = explode(',', $address);
      $province = trim($province);
    }

    $query = \Drupal::database()
      ->select('smallsteps_test_dashboard.Cities', 'c');
    $query->leftJoin('smallsteps_test_dashboard.Provinces', 'p', 'p.Id = c.Province');
    $query->addField('c', 'Name', 'city');
  ...
  public function getNearestLocationsInfo($lat, $lng, $services, $range, $page) {
    $account = \Drupal::currentUser();

    $limit = 5;
    $page = isset($page) ? $page : 0;
    $from = $page == 0 ? 0 : $page*$limit+1;
    $count = $page == 0 ? $limit+1 : $limit;
    $range = (isset($_GET['range']) && is_numeric($_GET['range']) && $_GET['range'] > 0) ? $_GET['range'] : $range;
    ...
    if ($range > 0) {
      $query->where('ROUND( 6371 * acos(cos(radians(:lat)) * cos(radians(l.Latitude)) * cos(radians(l.Longitude) - radians(:lng)) + sin(radians(:lat)) * sin(radians(l.Latitude))), 1 ) <= :range', [':range' => $range]);
    }
  ...
  public function getLocationsList($services = [], $lat = 0, $lng = 0, $range = 0) {
    $account = \Drupal::currentUser();
  ...
  public function getLocationBySearch($search, $type, $services = []) {
    ...
    elseif ($type == 'locatieadres') {
      ...
      if (preg_match('/^\d{4}/', $address)) {
        ...
        \Drupal::database()
          ->update('smallsteps_test_dashboard.Postcodes')
      ...
      elseif (stripos($address, ',') !== FALSE) {
        ...
        if ($result && $result['Latitude'] == 0 && $result['Longitude'] == 0) {
          ...
          if (is_array($response) && $response['status'] == 'OK' && is_array($response['results'])) {
            ...
            \Drupal::database()
              ->update('smallsteps_test_dashboard.Cities')
              ->fields([
